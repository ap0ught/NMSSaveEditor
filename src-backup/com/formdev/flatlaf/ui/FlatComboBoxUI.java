...
public class FlatComboBoxUI extends BasicComboBoxUI implements FlatStylingSupport.StyleableUI, FlatStylingSupport.StyleableLookupProvider {
         ...
         public void layoutContainer(Container parent) {
            ...
            if (FlatComboBoxUI.this.arrowButton != null && FlatComboBoxUI.this.comboBox.getFont() != null) {
               ...
               if (buttonWidth != FlatComboBoxUI.this.arrowButton.getWidth()) {
                  int xOffset = FlatComboBoxUI.this.comboBox.getComponentOrientation().isLeftToRight()
                     ? FlatComboBoxUI.this.arrowButton.getWidth() - buttonWidth
                     : 0;
                  FlatComboBoxUI.this.arrowButton
                     .setBounds(
                        FlatComboBoxUI.this.arrowButton.getX() + xOffset,
                        FlatComboBoxUI.this.arrowButton.getY(),
                        buttonWidth,
                        FlatComboBoxUI.this.arrowButton.getHeight()
                     );
   ...
   private void updateEditorPadding() {
      if (this.editor instanceof JTextField) {
         ...
         if (insets.top != 0 || insets.left != 0 || insets.bottom != 0 || insets.right != 0) {
            pad = new Insets(
               UIScale.unscale(Math.max(UIScale.scale(this.padding.top) - insets.top, 0)),
               UIScale.unscale(Math.max(UIScale.scale(this.padding.left) - insets.left, 0)),
               UIScale.unscale(Math.max(UIScale.scale(this.padding.bottom) - insets.bottom, 0)),
               UIScale.unscale(Math.max(UIScale.scale(this.padding.right) - insets.right, 0))
            );
         }
      ...
      if (paintBackground || c.isOpaque()) {
         ...
         if (enabled && !isCellRenderer) {
            Color buttonColor = paintButton
               ...
               : (
                  (this.buttonFocusedBackground != null || this.focusedBackground != null) && isPermanentFocusOwner(this.comboBox)
            ...
            if (buttonColor != null) {
               ...
               if (this.isMacStyle()) {
                  ...
                  FlatUIUtils.paintComponentBackground(
                     g2, arrowX + gap, insets.top + gap, arrowWidth - gap * 2, height - insets.top - insets.bottom - gap * 2, 0.0F, arc - focusWidth
                  );
               } else {
   ...
   private static class CellPaddingBorder extends AbstractBorder {
      ...
      public synchronized Insets getBorderInsets(Component c, Insets insets) {
         ...
         insets.left = insets.left + this.focusWidth;
         insets.right = insets.right + this.focusWidth;
         if (c instanceof JComponent) {
            Boolean macStyleHint = FlatClientProperties.clientPropertyBooleanStrict((JComponent)c, "FlatLaf.internal.FlatComboBoxUI.macStyleHint", null);
         ...
         if (c instanceof JComponent) {
            ...
            if (macStyleHint == Boolean.TRUE) {
               int ix = c.getComponentOrientation().isLeftToRight()
                  ? x + UIScale.scale(this.padding.left)
                  : x + width - UIScale.scale(this.padding.right) - FlatComboBoxUI.MacCheckedItemIcon.INSTANCE.getIconWidth();
   ...
   protected class FlatComboBoxButton extends FlatArrowButton {
      protected FlatComboBoxButton() {
         this(
            5,
            FlatComboBoxUI.this.arrowType,
            FlatComboBoxUI.this.buttonArrowColor,
            FlatComboBoxUI.this.buttonDisabledArrowColor,
            FlatComboBoxUI.this.buttonHoverArrowColor,
            null,
            FlatComboBoxUI.this.buttonPressedArrowColor,
            null
         );
      }
      ...
      protected Color getArrowColor() {
         return FlatComboBoxUI.this.isCellRenderer() && FlatComboBoxUI.this.isCellRendererBackgroundChanged()
            ? FlatComboBoxUI.this.comboBox.getForeground()
            : super.getArrowColor();
      }
      ...
      protected void paintArrow(Graphics2D g) {
         if (FlatComboBoxUI.this.isMacStyle() && !FlatComboBoxUI.this.comboBox.isEditable()) {
            ...
            FlatUIUtils.paintArrow(
               g, 0, 0, this.getWidth(), h, 1, this.chevron, this.getArrowWidth(), this.getArrowThickness(), this.getXOffset(), this.getYOffset() + 1.25F
            );
            FlatUIUtils.paintArrow(
               g,
               0,
               ...
               this.chevron,
               this.getArrowWidth(),
               this.getArrowThickness(),
               this.getXOffset(),
               this.getYOffset() - 1.25F
            );
         } else {
            super.paintArrow(g);
         }
   ...
   protected class FlatComboPopup extends BasicComboPopup {
      ...
      private boolean isPopupOverComboBox() {
         return FlatComboBoxUI.this.isMacStyle()
            && !this.comboBox.isEditable()
            && this.comboBox.getItemCount() > 0
            && this.comboBox.getItemCount() <= this.comboBox.getMaximumRowCount()
            && !FlatClientProperties.clientPropertyBoolean(this.comboBox, "JComboBox.isPopDown", false);
      }
