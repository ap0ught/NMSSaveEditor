...
public class FlatListUI extends BasicListUI implements FlatStylingSupport.StyleableUI {
   ...
   protected ListSelectionListener createListSelectionListener() {
      ...
      return e -> {
         ...
         if (this.useUnitedRoundedSelection(true, true)
            && !this.list.isSelectionEmpty()
            && this.list.getMaxSelectionIndex() - this.list.getMinSelectionIndex() >= 1) {
   ...
   protected void paintCell(
      Graphics g, final int row, final Rectangle rowBounds, ListCellRenderer cellRenderer, ListModel dataModel, ListSelectionModel selModel, int leadIndex
   ) {
      boolean isSelected = selModel.isSelectedIndex(row);
      final Component rendererComponent = cellRenderer.getListCellRendererComponent(
         this.list, dataModel.getElementAt(row), row, isSelected, FlatUIUtils.isPermanentFocusOwner(this.list) && row == leadIndex
      );
      ...
      if (isSelected
         && !isFileList
         && (rendererComponent instanceof DefaultListCellRenderer || rendererComponent instanceof BasicComboBoxRenderer)
         && (
            this.selectionArc > 0
               || this.selectionInsets != null
                  && (this.selectionInsets.top != 0 || this.selectionInsets.left != 0 || this.selectionInsets.bottom != 0 || this.selectionInsets.right != 0)
         )) {
   ...
   protected void paintCellSelection(Graphics g, int row, int x, int y, int width, int height) {
      ...
      FlatUIUtils.paintSelection(
         (Graphics2D)g, x, y, width, height, UIScale.scale(this.selectionInsets), arcTopLeft, arcTopRight, arcBottomLeft, arcBottomRight, 0
      );
   }

   private boolean useUnitedRoundedSelection(boolean vertical, boolean horizontal) {
      return this.selectionArc > 0
         && (
            this.selectionInsets == null
               || vertical && this.selectionInsets.top == 0 && this.selectionInsets.bottom == 0
               || horizontal && this.selectionInsets.left == 0 && this.selectionInsets.right == 0
         );
   }
