...
public class FlatPopupFactory extends PopupFactory {
   ...
   public Popup getPopup(Component owner, Component contents, int x, int y) throws IllegalArgumentException {
      ...
      } else if (!SystemInfo.isMacOS && !SystemInfo.isLinux) {
         int borderCornerRadius;
         if (isWindows11BorderSupported() && (borderCornerRadius = this.getBorderCornerRadius(owner, contents)) > 0) {
            FlatPopupFactory.NonFlashingPopup popup = new FlatPopupFactory.NonFlashingPopup(
               this.getPopupForScreenOfOwner(owner, contents, x, y, true), contents
            );
   ...
   private int getBorderCornerRadius(Component owner, Component contents) {
      String uiKey = contents instanceof BasicComboPopup
         ? "ComboBox.borderCornerRadius"
         : (
            contents instanceof JPopupMenu
               ? "PopupMenu.borderCornerRadius"
               : (contents instanceof JToolTip ? "ToolTip.borderCornerRadius" : "Popup.borderCornerRadius")
         );
   ...
   private Popup getHeavyWeightPopup(Component owner, Component contents, int x, int y) throws IllegalArgumentException {
      try {
         if (SystemInfo.isJava_9_orLater) {
            ...
            return (Popup)this.java9getPopupMethod.invoke((FlatPopupFactory)this, (Component)owner, (Component)contents, (int)x, (int)y, (boolean)true);
         } else {
            if (this.java8getPopupMethod == null) {
               Method m = PopupFactory.class.getDeclaredMethod("getPopup", Component.class, Component.class, int.class, int.class, int.class);
               m.setAccessible(true);
               this.java8getPopupMethod = MethodHandles.lookup().unreflect(m);
            }

            return (Popup)this.java8getPopupMethod.invoke((FlatPopupFactory)this, (Component)owner, (Component)contents, (int)x, (int)y, (int)2);
         }
   ...
   private class DropShadowPopup extends FlatPopupFactory.NonFlashingPopup {
      ...
      private Border createDropShadowBorder() {
         return new FlatDropShadowBorder(
            UIManager.getColor("Popup.dropShadowColor"), UIManager.getInsets("Popup.dropShadowInsets"), FlatUIUtils.getUIFloat("Popup.dropShadowOpacity", 0.5F)
         );
      }
