...
public class FlatSliderUI extends BasicSliderUI implements FlatStylingSupport.StyleableUI {
      ...
      if (c == null) {
         ...
         if (this.slider.getOrientation() == 1) {
            ...
            int centerSpacing = thumbHeight
               + (this.slider.getPaintTicks() ? this.getTickLength() : 0)
               + (this.slider.getPaintLabels() ? this.getHeightOfTallestLabel() : 0);
   ...
   public void paintTrack(Graphics g) {
      ...
      if (this.slider.getOrientation() == 0) {
         ...
         if (enabled && this.isRoundThumb()) {
            ...
            track = new Float(x, (float)this.trackRect.y, tw, (float)ch, tw, tw);
            coloredTrack = new Float(x, (float)(this.trackRect.y + ch), tw, (float)(this.trackRect.height - ch), tw, tw);
         } else {
            track = new Float(x, (float)this.trackRect.y, tw, (float)this.trackRect.height, tw, tw);
         }
      }
   ...
   public void paintThumb(Graphics g) {
      ...
      Color color = stateColor(
         this.slider, this.thumbHover, this.thumbPressed, thumbColor, this.disabledThumbColor, null, this.hoverThumbColor, this.pressedThumbColor
      );
      color = FlatUIUtils.deriveColor(color, thumbColor);
      Color foreground = this.slider.getForeground();
      Color borderColor = this.thumbBorderColor != null && foreground == this.defaultForeground
         ? stateColor(this.slider, false, false, this.thumbBorderColor, this.disabledThumbBorderColor, this.focusedThumbBorderColor, null, null)
         : null;
   ...
   public static void paintThumb(
      ...
      if (systemScaleFactor != 1.0 && systemScaleFactor != 2.0) {
         HiDPIUtils.paintAtScale1x(
            ...
            thumbRect.width,
            thumbRect.height,
            (g2d, x2, y2, width2, height2, scaleFactor) -> paintThumbImpl(
                  g,
                  slider,
                  x2,
                  y2,
                  ...
                  thumbBorderColor,
                  focusedColor,
                  (float)((double)thumbBorderWidth * scaleFactor),
   ...
   private static void paintThumbImpl(
      Graphics g,
      JSlider slider,
      int x,
      int y,
   ...
   public static Color stateColor(
      JSlider slider, boolean hover, boolean pressed, Color enabledColor, Color disabledColor, Color focusedColor, Color hoverColor, Color pressedColor
   ) {
   ...
   protected class FlatTrackListener extends TrackListener {
      ...
      public void mouseDragged(MouseEvent e) {
         ...
         if (FlatSliderUI.this.isDragging()
            && FlatSliderUI.this.slider.getSnapToTicks()
            && FlatSliderUI.this.slider.isEnabled()
