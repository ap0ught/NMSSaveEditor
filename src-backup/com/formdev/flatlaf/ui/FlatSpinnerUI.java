...
public class FlatSpinnerUI extends BasicSpinnerUI implements FlatStylingSupport.StyleableUI {
   ...
   private Component createArrowButton(int direction, String name) {
      FlatArrowButton button = new FlatArrowButton(
         direction, this.arrowType, this.buttonArrowColor, this.buttonDisabledArrowColor, this.buttonHoverArrowColor, null, this.buttonPressedArrowColor, null
      ) {
         @Override
   ...
   private void updateArrowButtonsStyle() {
      for (Component c : this.spinner.getComponents()) {
         if (c instanceof FlatArrowButton) {
            ((FlatArrowButton)c)
               .updateStyle(
                  this.arrowType, this.buttonArrowColor, this.buttonDisabledArrowColor, this.buttonHoverArrowColor, null, this.buttonPressedArrowColor, null
               );
         }
      }
   }
      ...
      if (paintButton && (handler.nextButton != null || handler.previousButton != null)) {
         ...
         if (isMacStyle) {
            ...
            FlatUIUtils.paintOutlinedComponent(
               g2, arrowX, by, arrowWidth, bh, 0.0F, 0.0F, 0.0F, lw, (float)UIScale.scale(12), null, separatorColor, this.buttonBackground
            );
   ...
   private class Handler implements LayoutManager, FocusListener, PropertyChangeListener {
      ...
      public Dimension preferredLayoutSize(Container parent) {
         ...
         return new Dimension(
            Math.max(
               ...
               UIScale.scale(minimumWidth) + Math.round(focusWidth * 2.0F)
            ),
      ...
      public void layoutContainer(Container parent) {
         ...
         if (this.nextButton == null && this.previousButton == null) {
            ...
            if (ltr) {
               buttonsRect.x = buttonsRect.x + editorRect.width + buttonsGap;
            } else {
               editorRect.x += buttonsWidth + buttonsGap;
            }

            if (isMacStyle) {
               int lineWidth = Math.round(FlatUIUtils.getBorderLineWidth(FlatSpinnerUI.this.spinner));
               if (lineWidth > 0) {
