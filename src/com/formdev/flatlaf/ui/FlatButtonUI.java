...
public class FlatButtonUI extends BasicButtonUI implements FlatStylingSupport.StyleableUI {
   ...
   public Object getStyleableValue(JComponent c, String key) {
      if (key.startsWith("help.")) {
         return this.helpButtonIcon instanceof FlatHelpButtonIcon
            ? ((FlatHelpButtonIcon)this.helpButtonIcon).getStyleableValue(key.substring("help.".length()))
            : null;
      } else {
   ...
   static boolean isIconOnlyOrSingleCharacterButton(Component c) {
      if (!(c instanceof JButton) && !(c instanceof JToggleButton)) {
         ...
         return icon != null && (text == null || text.isEmpty())
            || icon == null
               && text != null
               && ("...".equals(text) || text.length() == 1 || text.length() == 2 && Character.isSurrogatePair(text.charAt(0), text.charAt(1)));
      }
   }
      ...
      if (background != null) {
         ...
         try {
            ...
            if (this.paintShadow
               ...
               && c.isEnabled()
               && !isToolBarButton
               && !isBorderlessButton(c)
               && (!isFocusPainted(c) || !FlatUIUtils.isPermanentFocusOwner(c))) {
               g2.setColor(shadowColor);
               g2.fill(
                  new Float(
                     focusWidth,
                     focusWidth + UIScale.scale((float)this.shadowWidth),
                     (float)width - focusWidth * 2.0F,
                     (float)height - focusWidth * 2.0F,
                     arc,
                     arc
                  )
               );
      ...
      if (!isHelpButton(b)) {
         if (this.defaultBoldText && isDefaultButton(b) && b.getFont() instanceof UIResource) {
            ...
            if (boldWidth > textRect.width) {
               textRect.x = textRect.x - (boldWidth - textRect.width) / 2;
   ...
   protected Color getBackground(JComponent c) {
      ...
      if (((AbstractButton)c).isSelected()) {
         return buttonStateColor(
            c,
            toolBarButton ? this.toolbarSelectedBackground : this.selectedBackground,
            toolBarButton
               ? (this.toolbarDisabledSelectedBackground != null ? this.toolbarDisabledSelectedBackground : this.toolbarSelectedBackground)
               : this.disabledSelectedBackground,
            null,
            null,
            toolBarButton ? this.toolbarPressedBackground : this.pressedBackground
         );
         ...
         return buttonStateColor(
            c,
            this.getBackgroundBase(c, def),
            this.disabledBackground,
            this.isCustomBackground(c.getBackground()) ? null : (def ? this.defaultFocusedBackground : this.focusedBackground),
            def ? this.defaultHoverBackground : this.hoverBackground,
            def ? this.defaultPressedBackground : this.pressedBackground
         );
      }
   }
   ...
   protected Color getForeground(JComponent c) {
      ...
      if (((AbstractButton)c).isSelected()) {
         return buttonStateColor(
            c,
            toolBarButton ? (this.toolbarSelectedForeground != null ? this.toolbarSelectedForeground : c.getForeground()) : this.selectedForeground,
            toolBarButton
               ? (this.toolbarDisabledSelectedForeground != null ? this.toolbarDisabledSelectedForeground : this.disabledText)
               : (this.disabledSelectedForeground != null ? this.disabledSelectedForeground : this.disabledText),
            null,
            null,
            toolBarButton ? this.toolbarPressedForeground : this.pressedForeground
         );
      } else if (toolBarButton) {
         ...
         return buttonStateColor(
            c,
            this.getForegroundBase(c, def),
            this.disabledText,
            this.isCustomForeground(c.getForeground()) ? null : (def ? this.defaultFocusedForeground : this.focusedForeground),
            def ? this.defaultHoverForeground : this.hoverForeground,
            def ? this.defaultPressedForeground : this.pressedForeground
         );
      }
   }
   ...
   public Dimension getPreferredSize(JComponent c) {
      if (isHelpButton(c)) {
         ...
         if (prefSize == null) {
            ...
            prefSize.width = prefSize.width + this.defaultBoldPlainWidthDiff(c);
   ...
   protected class FlatButtonListener extends BasicButtonListener {
      // $VF: renamed from: b javax.swing.AbstractButton
      private final AbstractButton field_17;
      ...
      public void propertyChange(PropertyChangeEvent e) {
         ...
         FlatButtonUI.this.propertyChange(this.field_17, e);
      }
