    ...
    protected class FlatMenuRenderer extends FlatMenuItemRenderer
    {
        ...
        protected void paintBackground(final Graphics g) {
            ...
            if (((JMenu)this.menuItem).isTopLevelMenu() && this.isHover()) {
                final Color color = this.deriveBackground(this.getStyleFromMenuBarUI(ui -> ui.hoverBackground, this.hoverBackground));
        ...
        @Override
        protected void paintSelection(final Graphics g, Color selectionBackground, Insets selectionInsets, int selectionArc) {
            if (((JMenu)this.menuItem).isTopLevelMenu()) {
                ...
                final JMenuBar menuBar = (JMenuBar)this.menuItem.getParent();
                ...
                selectionArc = this.getStyleFromMenuBarUI(ui -> (ui.selectionArc != -1) ? Integer.valueOf(ui.selectionArc) : null, Integer.valueOf(this.menuBarSelectionArc));
            }
            super.paintSelection(g, selectionBackground, selectionInsets, selectionArc);
        }
        
        @Override
        protected void paintUnderlineSelection(final Graphics g, Color underlineSelectionBackground, Color underlineSelectionColor, int underlineSelectionHeight) {
            if (((JMenu)this.menuItem).isTopLevelMenu()) {
                ...
                underlineSelectionHeight = this.getStyleFromMenuBarUI(ui -> (ui.underlineSelectionHeight != -1) ? Integer.valueOf(ui.underlineSelectionHeight) : null, Integer.valueOf((this.menuBarUnderlineSelectionHeight != -1) ? this.menuBarUnderlineSelectionHeight : underlineSelectionHeight));
            }
            super.paintUnderlineSelection(g, underlineSelectionBackground, underlineSelectionColor, underlineSelectionHeight);
        }
        
        @Override
        protected void paintText(final Graphics g, final Rectangle textRect, final String text, Color selectionForeground, final Color disabledForeground) {
        ...
        private <T> T getStyleFromMenuBarUI(final Function<FlatMenuBarUI, T> f, final T defaultValue, final T defaultValue2) {
            return this.getStyleFromMenuBarUI(f, (defaultValue != null) ? defaultValue : defaultValue2);
        }
        
        private <T> T getStyleFromMenuBarUI(final Function<FlatMenuBarUI, T> f, final T defaultValue) {
            final MenuBarUI ui = ((JMenuBar)this.menuItem.getParent()).getUI();
            if (!(ui instanceof FlatMenuBarUI)) {
                return defaultValue;
            }
            final T value = f.apply((FlatMenuBarUI)ui);
