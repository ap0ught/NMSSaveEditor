...
public class FlatProgressBarUI extends BasicProgressBarUI implements FlatStylingSupport.StyleableUI {
   ...
   protected void installDefaults() {
      ...
      this.field_14 = UIManager.getInt("ProgressBar.arc");
   ...
   public void paint(Graphics g, JComponent c) {
      ...
      if (width > 0 && height > 0) {
         ...
         int arc = FlatClientProperties.clientPropertyBoolean(c, "JProgressBar.square", this.square)
            ? 0
            : Math.min(UIScale.scale(this.field_14), horizontal ? height : width);
         ...
         if (this.progressBar.isIndeterminate()) {
            ...
            if (this.boxRect != null) {
               ...
               ((Graphics2D)g)
                  .fill(new Float((float)this.boxRect.x, (float)this.boxRect.y, (float)this.boxRect.width, (float)this.boxRect.height, (float)arc, (float)arc));
            }
         } else {
            amountFull = this.getAmountFull(insets, width, height);
            Float progressShape = horizontal
               ? new Float(
                  c.getComponentOrientation().isLeftToRight() ? (float)x : (float)(x + (width - amountFull)),
                  (float)y,
                  (float)amountFull,
                  (float)height,
                  (float)arc,
               ...
               : new Float((float)x, (float)(y + (height - amountFull)), (float)width, (float)amountFull, (float)arc, (float)arc);
            g.setColor(this.progressBar.getForeground());
