...
public class FlatRadioButtonUI extends BasicRadioButtonUI implements FlatStylingSupport.StyleableUI {
   ...
   public Map<String, Class<?>> getStyleableInfos(JComponent c) {
      ...
      if (this.icon instanceof FlatCheckBoxIcon) {
         for (Entry<String, Class<?>> e : ((FlatCheckBoxIcon)this.icon).getStyleableInfos().entrySet()) {
   ...
   public Dimension getPreferredSize(JComponent c) {
      ...
      if (size == null) {
         ...
         if (focusWidth > 0) {
            ...
            size.width = size.width + Math.max(focusWidth - insets.left, 0) + Math.max(focusWidth - insets.right, 0);
            size.height = size.height + Math.max(focusWidth - insets.top, 0) + Math.max(focusWidth - insets.bottom, 0);
         }
   ...
   protected class FlatRadioButtonListener extends BasicButtonListener {
      ...
      private final AbstractButton field_16;
      ...
      public void propertyChange(PropertyChangeEvent e) {
         ...
         FlatRadioButtonUI.this.propertyChange(this.field_16, e);
      }
   }
}
