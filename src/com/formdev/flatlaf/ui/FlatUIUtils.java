...
public class FlatUIUtils {
   ...
   public static Insets addInsets(Insets insets1, Insets insets2) {
      if (insets1 == null) {
         ...
         return insets2 == null
            ? insets1
            : new Insets(insets1.top + insets2.top, insets1.left + insets2.left, insets1.bottom + insets2.bottom, insets1.right + insets2.right);
      }
   }
   ...
   public static boolean isCellEditor(Component c) {
      if (c == null) {
         ...
         return !"Table.editor".equals(name) && !"Tree.cellEditor".equals(name)
            ? c instanceof JComponent && Boolean.TRUE.equals(((JComponent)c).getClientProperty("JComboBox.isTableCellEditor"))
            : true;
      }
   }
   ...
   public static boolean isPermanentFocusOwner(Component c) {
      ...
      return c.hasFocus() ? true : keyboardFocusManager.getPermanentFocusOwner() == c && isInActiveWindow(c, keyboardFocusManager.getActiveWindow());
   }
   ...
   static void runWithLightAWTPeerUIDefaults(Runnable runnable) {
      if (lightAWTPeerDefaults == null) {
         FlatLaf lightLaf = (FlatLaf)(UIManager.getInt("Component.focusWidth") >= 2 ? new FlatIntelliJLaf() : new FlatLightLaf());
   ...
   public static void runWithoutRenderingHints(Graphics g, Object[] oldRenderingHints, Runnable runnable) {
      if (oldRenderingHints == null) {
         ...
         Object[] oldRenderingHints2 = new Object[]{
            g2.getRenderingHint(RenderingHints.KEY_ANTIALIASING), g2.getRenderingHint(RenderingHints.KEY_STROKE_CONTROL)
         };
   ...
   public static void paintOutlinedComponent(
      Graphics2D g,
      int x,
      int y,
      int width,
      ...
      if (systemScaleFactor != 1.0 && systemScaleFactor != 2.0) {
         HiDPIUtils.paintAtScale1x(
            ...
            width,
            height,
            (g2d, x2, y2, width2, height2, scaleFactor) -> paintOutlinedComponentImpl(
                  g2d,
                  x2,
                  y2,
                  width2,
                  height2,
                  (float)((double)focusWidth * scaleFactor),
                  focusWidthFraction,
                  (float)((double)focusInnerWidth * scaleFactor),
                  (float)((double)borderWidth * scaleFactor),
                  (float)((double)arc * scaleFactor),
                  focusColor,
                  borderColor,
                  background
               )
         ...
         paintOutlinedComponentImpl(
            g, x, y, width, height, focusWidth, focusWidthFraction, focusInnerWidth, borderWidth, arc, focusColor, borderColor, background
         );
      }
   }
      ...
      if (!(arcTopLeft > 0.0F) && !(arcTopRight > 0.0F) && !(arcBottomLeft > 0.0F) && !(arcBottomRight > 0.0F)) {
         ...
         if (systemScaleFactor != 1.0 && systemScaleFactor != 2.0) {
            HiDPIUtils.paintAtScale1x(
               ...
               width,
               height,
               (g2d, x2, y2, width2, height2, scaleFactor) -> paintRoundedSelectionImpl(
                     g2d,
                     x2,
                     y2,
                     width2,
                     height2,
                     (float)((double)arcTopLeft * scaleFactor),
                     (float)((double)arcTopRight * scaleFactor),
                     (float)((double)arcBottomLeft * scaleFactor),
   ...
   private static void paintRoundedSelectionImpl(
      Graphics2D g, int x, int y, int width, int height, float arcTopLeft, float arcTopRight, float arcBottomLeft, float arcBottomRight
   ) {
   ...
   public static Path2D createRoundRectangle(
      float x, float y, float width, float height, float lineWidth, float arcTopLeft, float arcTopRight, float arcBottomLeft, float arcBottomRight
   ) {
      Path2D path = new Float(0);
      ...
      path.append(
         createRoundRectanglePath(
            ...
            height - lineWidth * 2.0F,
            arcTopLeft - lineWidth,
            arcTopRight - lineWidth,
            arcBottomLeft - lineWidth,
            arcBottomRight - lineWidth
         ),
         false
      );
      return path;
   ...
   public static Shape createRoundRectanglePath(
      float x, float y, float width, float height, float arcTopLeft, float arcTopRight, float arcBottomLeft, float arcBottomRight
   ) {
      if (arcTopLeft <= 0.0F && arcTopRight <= 0.0F && arcBottomLeft <= 0.0F && arcBottomRight <= 0.0F) {
   ...
   public static Shape createRoundTrianglePath(float x1, float y1, float x2, float y2, float x3, float y3, float arc) {
      double averageSideLength = (
            distance((double)x1, (double)y1, (double)x2, (double)y2)
               + distance((double)x2, (double)y2, (double)x3, (double)y3)
               + distance((double)x3, (double)y3, (double)x1, (double)y1)
         )
         / 3.0;
      double t1 = 1.0 / averageSideLength * (double)arc;
      double t2 = 1.0 - t1;
      return createPath(
         lerp((double)x3, (double)x1, t2),
         lerp((double)y3, (double)y1, t2),
         -1.000000000002E12,
         (double)x1,
         (double)y1,
         lerp((double)x1, (double)x2, t1),
         lerp((double)y1, (double)y2, t1),
         lerp((double)x1, (double)x2, t2),
         lerp((double)y1, (double)y2, t2),
         -1.000000000002E12,
         (double)x2,
         (double)y2,
         lerp((double)x2, (double)x3, t1),
         lerp((double)y2, (double)y3, t1),
         lerp((double)x2, (double)x3, t2),
         lerp((double)y2, (double)y3, t2),
         -1.000000000002E12,
         (double)x3,
         (double)y3,
         lerp((double)x3, (double)x1, t1),
         lerp((double)y3, (double)y1, t1)
      );
   }

   public static void paintArrow(
      Graphics2D g, int x, int y, int width, int height, int direction, boolean chevron, int arrowSize, float arrowThickness, float xOffset, float yOffset
   ) {
   ...
   public static Shape createArrowShape(int direction, boolean chevron, float w, float h) {
      switch (direction) {
         case 1:
            return createPath(!chevron, 0.0, (double)h, (double)(w / 2.0F), 0.0, (double)w, (double)h);
         case 2:
         case 4:
         case 6:
         default:
            return new Float();
         case 3:
            return createPath(!chevron, 0.0, 0.0, (double)w, (double)(h / 2.0F), 0.0, (double)h);
         case 5:
            return createPath(!chevron, 0.0, 0.0, (double)(w / 2.0F), (double)h, (double)w, 0.0);
         case 7:
            return createPath(!chevron, (double)w, 0.0, 0.0, (double)(h / 2.0F), (double)w, (double)h);
      }
   }
   ...
   public static ComponentUI createSharedUI(Object key, Supplier<ComponentUI> newInstanceSupplier) {
      return !useSharedUIs
         ? newInstanceSupplier.get()
         : sharedUIinstances.computeIfAbsent(UIManager.getLookAndFeel(), k -> new IdentityHashMap<>()).computeIfAbsent(key, k -> newInstanceSupplier.get());
   }
