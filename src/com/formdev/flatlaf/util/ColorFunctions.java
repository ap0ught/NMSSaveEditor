...
public class ColorFunctions {
   ...
   // $VF: renamed from: mix (java.awt.Color, java.awt.Color, float) java.awt.Color
   public static Color method_52(Color color1, Color color2, float weight) {
      if (weight >= 1.0F) {
         return color1;
      } else if (weight <= 0.0F) {
         return color2;
         ...
         return new Color(
            Math.round((float)r2 + (float)(r1 - r2) * weight),
            Math.round((float)g2 + (float)(g1 - g2) * weight),
            Math.round((float)b2 + (float)(b1 - b2) * weight),
            Math.round((float)a2 + (float)(a1 - a2) * weight)
         );
      }
   }

   public static Color tint(Color color, float weight) {
      return method_52(Color.white, color, weight);
   }

   public static Color shade(Color color, float weight) {
      return method_52(Color.black, color, weight);
   }
   ...
   public static Color applyFunctions(Color color, ColorFunctions.ColorFunction... functions) {
      if (functions.length == 1 && functions[0] instanceof ColorFunctions.class_436) {
         ColorFunctions.class_436 mixFunction = (ColorFunctions.class_436)functions[0];
         return method_52(color, mixFunction.color2, mixFunction.weight / 100.0F);
      } else {
         float[] hsl = HSLColor.fromRGB(color);
         float alpha = (float)color.getAlpha() / 255.0F;
         float[] hsla = new float[]{hsl[0], hsl[1], hsl[2], alpha * 100.0F};

         for (ColorFunctions.ColorFunction function : functions) {
            function.apply(hsla);
         }
   ...
   public static class class_436 implements ColorFunctions.ColorFunction {
      public final Color color2;
      public final float weight;

      public class_436(Color color2, float weight) {
         this.color2 = color2;
         this.weight = weight;
      }
      ...
      public void apply(float[] hsla) {
         ...
         Color color = ColorFunctions.method_52(color1, this.color2, this.weight / 100.0F);
         float[] hsl = HSLColor.fromRGB(color);
