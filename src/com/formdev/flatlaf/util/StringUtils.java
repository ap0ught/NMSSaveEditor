   ...
   public static List<String> split(String str, char delim, boolean trim, boolean excludeEmpty) {
      ...
      if (delimIndex < 0) {
         ...
         for (index = 0; delimIndex >= 0; delimIndex = str.indexOf(delim, index)) {
            method_50(strs, str, index, delimIndex, trim, excludeEmpty);
            index = delimIndex + 1;
         }

         method_50(strs, str, index, str.length(), trim, excludeEmpty);
         return strs;
      }
   }

   // $VF: renamed from: add (java.util.List, java.lang.String, int, int, boolean, boolean) void
   private static void method_50(List<String> strs, String str, int beginIndex, int endIndex, boolean trim, boolean excludeEmpty) {
      if (trim) {
