...
public final class FormLayout implements LayoutManager2, Serializable {
   ...
   private void shiftComponentsHorizontally(int columnIndex, boolean remove) {
      ...
      for (Entry entry : this.constraintMap.entrySet()) {
         ...
         if (x1 == columnIndex && remove) {
            throw new IllegalStateException(
               "The removed column " + columnIndex + " must not contain component origins.\n" + "Illegal component=" + entry.getKey()
            );
         }
   ...
   public void layoutContainer(Container parent) {
      synchronized (parent.getTreeLock()) {
         ...
         int[] x = this.computeGridOrigins(
            parent, totalWidth, insets.left, this.colSpecs, this.colComponents, this.colGroupIndices, this.minimumWidthMeasure, this.preferredWidthMeasure
         );
         int[] y = this.computeGridOrigins(
            parent, totalHeight, insets.top, this.rowSpecs, this.rowComponents, this.rowGroupIndices, this.minimumHeightMeasure, this.preferredHeightMeasure
         );
   ...
   private Dimension computeLayoutSize(Container parent, FormLayout.Measure defaultWidthMeasure, FormLayout.Measure defaultHeightMeasure) {
      synchronized (parent.getTreeLock()) {
         this.initializeColAndRowComponentLists();
         int[] colWidths = this.maximumSizes(
            parent, this.colSpecs, this.colComponents, this.minimumWidthMeasure, this.preferredWidthMeasure, defaultWidthMeasure
         );
         int[] rowHeights = this.maximumSizes(
            parent, this.rowSpecs, this.rowComponents, this.minimumHeightMeasure, this.preferredHeightMeasure, defaultHeightMeasure
         );
         ...
         int width1 = method_61(groupedWidths);
         int height1 = method_61(groupedHeights);
         int maxWidth = width1;
   ...
   private int[] computeGridOrigins(
      Container container,
      int totalSize,
      int offset,
      List formSpecs,
      ...
      int totalMinSize = method_61(groupedMinSizes);
      int totalPrefSize = method_61(groupedPrefSizes);
      ...
      int totalGroupedSize = method_61(groupedSizes);
   ...
   private void layoutComponents(int[] x, int[] y) {
      ...
      for (Entry entry : this.constraintMap.entrySet()) {
         ...
         constraints.setBounds(
            component, this, cellBounds, this.minimumWidthMeasure, this.minimumHeightMeasure, this.preferredWidthMeasure, this.preferredHeightMeasure
         );
      }
   }
   ...
   // $VF: renamed from: sum (int[]) int
   private static int method_61(int[] sizes) {
      int sum = 0;
