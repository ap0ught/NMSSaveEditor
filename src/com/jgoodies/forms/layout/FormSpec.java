...
public abstract class FormSpec implements Serializable {
   ...
   private static final FormSpec.DefaultAlignment[] VALUES = new FormSpec.DefaultAlignment[]{
   ...
   private static double parseResizeWeight(String token) {
      if (token.equals("g") || token.equals("grow")) {
         return 1.0;
      } else if (!token.equals("n") && !token.equals("nogrow") && !token.equals("none")) {
         if ((token.startsWith("grow(") || token.startsWith("g(")) && token.endsWith(")")) {
            ...
            throw new IllegalArgumentException(
               "The resize argument '" + token + "' is invalid. " + " Must be one of: grow, g, none, n, grow(<double>), g(<double>)"
            );
         }
      } else {
         return 0.0;
