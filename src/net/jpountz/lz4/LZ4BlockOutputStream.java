...
public class LZ4BlockOutputStream extends FilterOutputStream {
   ...
   public void write(int b) throws IOException {
      ...
      if (this.field_138 == this.blockSize) {
         this.flushBufferedData();
      }

      this.buffer[this.field_138++] = (byte)b;
   }
   ...
   public void write(byte[] b, int off, int len) throws IOException {
      ...
      while (this.field_138 + len > this.blockSize) {
         int l = this.blockSize - this.field_138;
         System.arraycopy(b, off, this.buffer, this.field_138, this.blockSize - this.field_138);
         this.field_138 = this.blockSize;
         this.flushBufferedData();
         off += l;
         len -= l;
      }

      System.arraycopy(b, off, this.buffer, this.field_138, len);
      this.field_138 += len;
   }
   ...
   private void flushBufferedData() throws IOException {
      if (this.field_138 != 0) {
         this.checksum.reset();
         this.checksum.update(this.buffer, 0, this.field_138);
         int check = (int)this.checksum.getValue();
         int compressedLength = this.compressor.compress(this.buffer, 0, this.field_138, this.compressedBuffer, HEADER_LENGTH);
         int compressMethod;
         if (compressedLength >= this.field_138) {
            compressMethod = 16;
            compressedLength = this.field_138;
            System.arraycopy(this.buffer, 0, this.compressedBuffer, HEADER_LENGTH, this.field_138);
         } else {
            compressMethod = 32;
         }
         ...
         writeIntLE(this.field_138, this.compressedBuffer, MAGIC_LENGTH + 5);
