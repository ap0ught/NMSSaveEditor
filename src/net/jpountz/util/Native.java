...
public enum Native {
   ...
   // $VF: renamed from: os () net.jpountz.util.Native$OS
   private static Native.class_434 method_843() {
      String osName = System.getProperty("os.name");
      if (osName.contains("Linux")) {
         return Native.class_434.LINUX;
      } else if (osName.contains("Mac")) {
         return Native.class_434.field_694;
      } else if (osName.contains("Windows")) {
         return Native.class_434.WINDOWS;
      } else if (!osName.contains("Solaris") && !osName.contains("SunOS")) {
         throw new UnsupportedOperationException("Unsupported operating system: " + osName);
      } else {
         return Native.class_434.SOLARIS;
      }
   }

   private static String resourceName() {
      Native.class_434 os = method_843();
      ...
      if (!loaded) {
         ...
         try {
            ...
            if (is == null) {
               ...
               try {
                  tempLibLock = File.createTempFile("liblz4-java-", "." + method_843().libExtension + ".lck");
   ...
   private static enum class_434 {
      WINDOWS("win32", "so"),
      LINUX("linux", "so"),
      // $VF: renamed from: MAC net.jpountz.util.Native$OS
      field_694("darwin", "dylib"),
      SOLARIS("solaris", "so");

      public final String name;
      public final String libExtension;

      private class_434(String name, String libExtension) {
         this.name = name;
