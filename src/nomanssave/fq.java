...
public interface fq {
   ...
   static fq a(File var0, fR var1) {
      if (!var0.exists()) {
         ...
         try {
            if (var0.isDirectory()) {
               if (var0.listFiles(var0x -> var0x.getName().equalsIgnoreCase("containers.index")).length > 0) {
                  return new fT(var0, var1);
               }

               if (var0.listFiles(var0x -> var0x.getName().equalsIgnoreCase("accountdata.hg") || Pattern.matches("save\\d*.hg", var0x.getName().toLowerCase())).length
                  > 0) {
                  return new fJ(var0, var1);
               }

               if (var0.listFiles(var0x -> Pattern.matches("savedata\\d{2}.hg", var0x.getName().toLowerCase())).length > 0) {
   ...
   default ft[] bV() {
      return Arrays.asList(this.bU()).stream().filter(var0 -> !var0.isEmpty()).toArray(ft[]::new);
   }
